...
class AKTConfig:
    ...
    feature_mode: str = "problem_onehot"  # problem_onehot | problem_plus_q
    model_name: str = "akt"
    seed: int = 0
    epochs: int = 8
    batch_size: int = 64
    ...
    out_dir: str = "runs/akt_first"


def run_akt(cfg: AKTConfig) -> dict:
    ...
    if cfg.feature_mode not in FEATURE_MODES:
        raise ValueError(f"Unsupported AKT feature_mode='{cfg.feature_mode}'.")
    ...
    evidence_dim = interaction_evidence_dim(cfg.feature_mode, q_kc_dim)
    ...
    max_train_len = max(int(s[1].item()) for s in train_samples + valid_samples + test_samples)
    ...
    backbone = AKT(
        ...
        monotonic_rate=cfg.monotonic_rate,
    )
