"""Train/eval KCGen-KT-lite on CSEDM framework logs."""
...
def _run_kcgen_epoch(model, loader, optimizer, device) -> EpochMetrics:
    ...
    for prob, code, resp, lengths, target_q, y in loader:
        ...
        with torch.set_grad_enabled(train):
            logits = model(code, prob, resp, lengths, target_q)
            loss = bce(logits, y)
            if train:
                loss.backward()
                optimizer.step()
...
def run_kcgen_kt_lite(cfg: IICELiteConfig) -> dict:
    ...
    if not train_samples or not valid_samples or not test_samples:
        raise RuntimeError("Empty train/valid/test samples for KCGen-KT-lite.")
    ...
    for ep in range(1, cfg.epochs + 1):
        tr = _run_kcgen_epoch(model, train_loader, optimizer, device)
        va = _run_kcgen_epoch(model, valid_loader, None, device)
    ...
    te = _run_kcgen_epoch(model, test_loader, None, device)

    summary = {
        "config": asdict(cfg),
        "model_name": "kcgen_kt_lite",
        "device": str(device),
        "q_dim": q_dim,
        "codebert_dim": CODEBERT_VECTOR_DIM,
        "note": "KCGen-KT-lite: human 18-KC + CodeBERT + explicit mastery; correctness-only",
        "split_sizes": {
